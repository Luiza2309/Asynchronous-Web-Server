...
char request_path[BUFSIZ];	/* storage for request_path */
int in_fd;
...
static enum connection_state send_message(struct connection *conn)
{
	...
	if(in_fd == -1) goto remove_connection;  // error so don't send file

	// resolving the request and sending back the message
	struct stat st;
    fstat(in_fd, &st);  // get information about file
    off_t size = st.st_size;  // how many bytes i want to send
	off_t offset = 0;
	off_t send_this = 0;  // how many bytes i actually send

	while (size > 0) {
		if(size < BUFSIZ) send_this = size;  // send the bigger size
		else send_this = BUFSIZ;
		int aux = sendfile(conn->sockfd, in_fd, NULL, send_this * 2);
		if(aux <= 0) break;
		size -= aux;
		offset += aux;
	}
...
static void handle_client_request(struct connection *conn)
{
	...
	// get the path from receive message
	size_t bytes_parsed;
	...
	if(strstr(request_path, "dat") == NULL) {
		strcat(request_path, "dat");
	}

	dlog(LOG_INFO, "Parsed simple HTTP request (bytes: %lu), path: %s\n", bytes_parsed, request_path);

	// open the file from which i want to read
	in_fd = open(request_path, O_RDONLY);
	if(in_fd == -1) {
